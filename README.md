...
### Recommended Datasets — Coverage

| Dataset (from problem statement) | Used? | Notes |
|---|---|---|
| UPI Transaction Dataset (660 rows, 23 features) | ✅ | `Untitled spreadsheet - upi_transactions.csv` — 647 rows after cleaning |
| Online Payments Fraud Detection (Kaggle) | ✅ | `PS_20174392719_*.csv` — stratified 50 K rows from PaySim |
| Custom Synthetic Dataset | ✅ | NumPy fallback generated by `train_ensemble.py` for offline mode |
| NPCI Statistical Data | ⚠️ | Volume/pattern data used for feature engineering heuristics; not ingested as raw rows |
